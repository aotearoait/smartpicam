...
class ModerateLatencySmartPiCam:
    ...
    def setup_logging(self):
        ...
        self.logger.info("SmartPiCam v2.5 - Moderate Latency (simple approach)")
    ...
    def load_config(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Moderate latency mode: Simple UDP approach")
    ...
    def _build_moderate_latency_ffmpeg_command(self) -> List[str]:
        """Build moderate latency FFmpeg command - simple UDP approach"""
        if not self.cameras:
            return []
        
        # Start with the EXACT working command structure but change transport
        ...
        for camera in self.cameras:
            ...
            if camera in self.working_cameras:
                # Working camera with simple UDP (main change from working version)
                cmd.extend([
                    "-rtsp_transport", "udp",  # ONLY change: UDP instead of TCP
                    "-i", camera.url
                ])
        ...
        # Build filter complex - EXACTLY like the working version
        filter_parts = []
    ...
    def _test_single_camera(self, camera: Camera) -> bool:
        ...
        test_cmd = [
            ...
            "-rtsp_transport", "tcp",  # Use TCP for testing (more reliable)
            "-i", camera.url,
            "-t", "1",
            "-f", "null", "-"
        ]
    ...
    def start_display(self) -> bool:
        """Start the moderate latency FFmpeg grid display"""
        ...
        self.logger.info("Starting moderate latency FFmpeg display (UDP transport)...")
...
def main():
    ...
    app = ModerateLatencySmartPiCam()
    app.logger.info("Starting Moderate Latency SmartPiCam...")
